...
3. 了解环境变量和启动参数
   ...
   环境变量是 Linux 配置输入法时较复杂的部分，但绝对值得一学。假设我们输入一个字母`x`，通常情况下软件会显示出这个`x`，但是设置环境变量之后，这个`x`会被输入模块接管，经由环境变量指定的通信机制派发给输入法进行处理，所以如果你的输入法出现异常，九成是环境变量配置错误导致软件和输入法没能正确通信。不同工具开发的应用对应的环境变量不同，需要正确设置，以下是最常见的环境变量：

   - `GTK_IM_MODULE` 适用于 GTK2/3/4 应用和以 X11/XWayland 运行的 Electron/Chromium 应用。

   - `QT_IM_MODULE` 适用于 Qt 应用。

   - `XMODIFIERS` 在 Wayland 环境下仅适用于 XWayland 兼容层运行的 Xlib/GTK2/Qt4 应用。

   - `SDL_IM_MODULE` 适用于 SDL2 应用。

   - `--enable-features=UseOzonePlatform --ozone-platform=wayland --enable-wayland-ime --wayland-text-input-version=3` 

     这一项比较特殊，不是环境变量，而是 Electron/Chromium 应用的命令行参数，典型的 Electron 应用有LinuxQQ、夸克、百度网盘等。原理是让软件以 Wayland 运行，走 Wayland 的协议。不用全写，通常只写`--ozone-platform=wayland`就行，有问题再说。
...
4. 正确设置环境变量和启动参数
    ...
    在 Wayland 环境下，理论上输入法开箱即用，走的是 Wayland 的 `text-input` 协议，所以**Wayland 软件不需要设置任何环境变量**。我们真正需要处理的只有那些在 Wayland 环境下走 XWayland 兼容层运行的 X11 应用和极个别 Qt 应用。
